...
PROPERTY_IMU_DATA       = 'imu_data'         # Get/Set IMU data
...
class IMU_APP(API):
    """
    Class responsible for enforcing the IMU API to transport IMU data to a client application.

    The API defines the structure and rules for API messages between systems.
    ...
    def validate(self, api_msg: Dict[str, Any]):
        ...
        if 'api_call' not in api_msg:
            raise XAPIValidationFailed("Message missing required field 'api_call'")

        # API call is contained within the API message
        api_call = api_msg['api_call'] 
        if not isinstance(api_call, dict):
            raise XAPIValidationFailed("Message field 'api_call' must be a dictionary")
