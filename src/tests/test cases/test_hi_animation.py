...
def test_drift_scan_frame_uses_scan_midpoint_and_spectral_metadata():
    ...
    scan = SimpleNamespace(scan_model=scan_model, mpr=np.arange(8, dtype=float))
    ...
    assert frames[0].velocity_lsr_kms.shape == (8,)
    assert frames[0].spectrum.shape == (8,)
    assert np.all(np.diff(frames[0].velocity_lsr_kms) >= 0)
    assert np.all(np.isfinite(frames[0].velocity_lsr_kms))
...
def test_create_animation_writes_gif(monkeypatch, tmp_path):
    fake_animation_shin = ModuleType("obs.animation_shin")
    fake_animation_shin.generate_star_chart = lambda *args, **kwargs: np.ones(
        (16, 16, 3), dtype=np.float32
    )
    monkeypatch.setitem(sys.modules, "obs.animation_shin", fake_animation_shin)
    ...
    frame = AnimationFrame(
        ...
        galactic_b_deg=10.0,
        velocity_lsr_kms=np.linspace(-100.0, 100.0, 8),
