...
build_cmd() {
    ...
    printf 'cd "%s" && %sexport PYTHONPATH="%s:${PYTHONPATH:-}" && "%s/%s"; exec bash' \
        "$SESSION_DIR" "$setup_cmd" "$SESSION_DIR" "$SCRIPT_DIR" "$script_name"
}

CMD1="$(build_cmd "ws001.sh")"
CMD2="$(build_cmd "mdsim.sh")"
CMD3="$(build_cmd "dig001.sh")"
